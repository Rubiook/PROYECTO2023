...
{
    public class LotesAsignados
    {
        private readonly RepositorioUsuarios repositorioUsuarios;

        private MySqlConnection _connection;
        public LotesAsignados()
        {
        ...
        */
        ...
        //REVERIFICAR LOTE ASIGNADO--------------------------------------------------------------
        public bool ReVerificarLoteAsignado(int loteId)
        {
            using (MySqlConnection connection = Conexion.obtenerConexion())
            {
                try
                {
                    string query = "SELECT COUNT(*) FROM lotesasignados WHERE id_lote = @LoteId AND id_remate = (SELECT id_remate FROM lote WHERE id = @LoteId)";
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@LoteId", loteId);
                        int count = Convert.ToInt32(command.ExecuteScalar());

                        return count > 0; // Retorna true si el lote está asignado correctamente, false si no lo está
                    }
                }
                catch (MySqlException ex)
                {
    ...
    public void EliminarRemate(int remateId)
    {
        ...
        public void ActualizarImagenLote(int loteId, Image imagen)
        {
            byte[] imagenData;
            ...
            if (imagenData.Length <= 10485760)
            {
                try
                {
                    ...
                    // Puedes lanzar una excepción personalizada si necesitas manejar errores específicos.
                }
                catch (MySqlException ex)
                {
                    throw new Exception("Error al actualizar la imagen del lote: " + ex.Message);
                }
            }
            else
            {
